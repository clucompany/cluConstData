...
macro_rules! const_single_data {
	...
	[& $l: lifetime str = $a:expr, $($b:expr),*] => {{
		const _HIDDEN: & $l str = $crate::concat_const_raw!(@as_bytes: $a $(, $b)*);
		_HIDDEN
	...
	[& $l: lifetime [$type: ty] = $a:expr, $($b:expr),*] => {{
		const _HIDDEN: & $l [$type] = &$crate::concat_const_raw!($type: $a $(, $b)*);
		_HIDDEN
	...
	[& $l: lifetime [$type: ty; $size:expr] = $a:expr, $($b:expr),*] => {{
		const _HIDDEN: & $l [$type; $size] = &$crate::concat_const_raw!($type: $a $(, $b)*);
		_HIDDEN
	...
	[[$type: ty; $size:expr] = $a:expr, $($b:expr),*] => {{
		const _HIDDEN: [$type; $size] = $crate::concat_const_raw!($type: $a $(, $b)*);
		_HIDDEN
	...
	[[$type: ty] = $a:expr, $($b:expr),*] => {{
		const _HIDDEN: [$type] = $crate::concat_const_raw!($type: $a $(, $b)*);
		_HIDDEN
...
macro_rules! let_single_data {
	...
	//&'lifetime str
	[& $l: lifetime str = $a:expr, $($b:expr),*] => {{
		let data: & $l str = $crate::concat_const_raw!(@as_bytes: $a $(, $b)*);
		data
	}};

	//&'lifetime [u8]
	[& $l: lifetime [$type: ty] = $a:expr, $($b:expr),*] => {{
		let data: & $l [$type] = &$crate::concat_const_raw!($type: $a $(, $b)*);
		data
	}};

	//&'lifetime [u8]
	[& $l: lifetime [$type: ty; $size:expr] = $a:expr, $($b:expr),*] => {{
		let data: & $l [$type; $size] = &$crate::concat_const_raw!($type: $a $(, $b)*);
		data
	}};
	//
	...
	//[u8; 10]
	[[$type: ty; $size:expr] = $a:expr, $($b:expr),*] => {{
		let data: [$type; $size] = $crate::concat_const_raw!($type: $a $(, $b)*);
		data
	}};

	//[u8]
	[[$type: ty] = $a:expr, $($b:expr),*] => {{
		let data: [$type] = $crate::concat_const_raw!($type: $a $(, $b)*);
		data
	}};
